...
class CoreDataMigrator: CoreDataMigratorProtocol {
    ...
    func requiresMigration(at storeURL: URL, toVersion version: CoreDataMigrationVersion) -> Bool {
        ...
        let currentVersion = CoreDataMigrationVersion.compatibleVersionForStoreMetadata(metadata)
    ...
    func migrateStore(at storeURL: URL, toVersion version: CoreDataMigrationVersion) {
        ...
        for migrationStep in migrationSteps {
            print("MIGRATING FROM \(migrationStep.sourceVersion.rawValue) TO \(migrationStep.destinationVersion.rawValue)")
            ...
            guard let metadata = NSPersistentStoreCoordinator.metadata(at: currentURL), let version = CoreDataMigrationVersion.compatibleVersionForStoreMetadata(metadata) else {
                fatalError("unknown store version at URL \(storeURL)")
            }
            print("AFTER MIGRATION VERSION: \(version.rawValue)")
        }
